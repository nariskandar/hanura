...
<html>
    <head>
        ...
        <link href="<?= base_url('assets/plugins/bootstrap-datepicker/css/datepicker3.css'); ?>" rel="stylesheet"
        	type="text/css" />
        <link href="<?= base_url('assets/plugins/bootstrap-colorpicker/css/colorpicker.css'); ?>" rel="stylesheet"
        	type="text/css" />
        ...
        <link href="<?= base_url('assets/css/custom.css'); ?>" rel="stylesheet" type="text/css" />
		    ...
		    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
        <script src="<?php echo base_url()?>node_modules/echarts/dist/echarts.min.js"></script>
...
<script>
  $(document).ready(function () {
  	...
  	function getNomerSurat() {
  		if ($("#calon").val() != '') {
  			var nosurat = $("#calon").val();
  			$.ajax({
  				type: "GET",
  				url: "<?= site_url('rekom/add_ajax_nomersurat/');?>" + nosurat,
  				data: {
  					// "id_calon": $("#calon").val()
  				},
  				success: function (resp) {
  	...
  	$("#partai").change(function () {
  		$.ajax({
  			...
  			success: function (resp) {
  				...
  				// $("#no_surat").html(resp)
  				// $("#total_kursi").html(resp['data']),
  				// console.log(resp['data']),
  				// console.log(resp['data1'])
  				// $("#no_surat").html(resp['data1'])
  			},
  			error: function () {}
  		})
  		return false;
